...
public class EventDispatcherService {
    ...
    private Brand resolveBrand(JsonNode p) {
        ...
        if (brand == null && p.hasNonNull("brand") && p.get("brand").isObject()) {
            JsonNode b = p.get("brand");
            if (b.hasNonNull("id")) brand = brandRepository.findById(b.get("id").asInt()).orElse(null);
            if (brand == null && b.hasNonNull("name")) brand = brandRepository.findByNameIgnoreCase(b.get("name").asText());
        }
        return brand;
    }
            ...
            for (JsonNode c : p.get("categories")) {
                ...
                if (c.isInt()) code = c.asInt();
                    ...
                    if (c.hasNonNull("code")) code = c.get("code").asInt();
                        ...
                        Category cat = categoryRepository.findById(c.get("id").asInt()).orElse(null);
