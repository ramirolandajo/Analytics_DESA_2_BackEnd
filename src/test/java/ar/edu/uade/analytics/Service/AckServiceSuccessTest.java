...
class AckServiceSuccessTest {
    ...
    @Test
    void sendAck_success_callsCoreAndMarksTrue() {
        String eid = "evt-1";
        when(idempotencyService.findByEventId(eid)).thenReturn(Optional.of(new ar.edu.uade.analytics.Entity.ConsumedEventLog()));
        when(coreApiClient.ackEvent(eq(eid), any())).thenReturn(new CoreApiClient.AckResponse(true, null));

        svc.sendAck(eid);

        verify(coreApiClient).ackEvent(eq(eid), any());
        verify(idempotencyService).markAck(eq(eid), eq(true), isNull());
    }
}
